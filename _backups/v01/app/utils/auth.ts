...
const createAuditLog = async (
  ...
  try {
    // userId가 있는 경우와 없는 경우를 나누어 처리
    if (userId) {
      await prisma.$executeRaw`
        INSERT INTO "audit_logs" (
          "id", "user_id", "action", "timestamp", "ip_address", "details"
        ) VALUES (
          uuid_generate_v4(), ${userId}::uuid, ${action}, NOW(), ${ipAddress}, ${details ? JSON.stringify(details) : null}::json
        )
      `;
    } else {
      await prisma.$executeRaw`
        INSERT INTO "audit_logs" (
          ...
          uuid_generate_v4(), ${action}, NOW(), ${ipAddress}, ${details ? JSON.stringify(details) : null}::json
        )
      `;
    }
  } catch (error) {
